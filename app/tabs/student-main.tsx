    ...
    <View style={styles.container}>
        ...
        {notifications.length > 0 ? (
          notifications.map((notification) => (
            ...
            >
              ...
              >
                <View>
                  <Text>{notification.message} <Text style={styles.bold}>{notification.subject}</Text></Text>
                </View>
      ...
      <ScrollView style={styles.section}>
        ...
        {subjects.map((subject) => (
          <TouchableOpacity
            key={subject.id}
            onPress={() => { console.log(`Tapped on subject: ${subject.name}`); }}
            style={styles.subject}
          >
            <View style={styles.subjectContent}>
              <View>
                <Text style={styles.subjectName}>{subject.name}</Text>
                <Text style={styles.subjectTasks}>Cantidad de Tareas ({subject.taskCount}/{subject.totalTasks})</Text>
              </View>
